...
export async function generateEpub(
  ...
  if (bookInfo.description) {
    epubChapters.push({
      title: '서문',
      content: `<h1>서문</h1><p>${bookInfo.description}</p>`,
    });
  }
  ...
  for (const chapter of chapters) {
    ...
    if (chapter.preface) {
      chapterContent += `<blockquote><em>${chapter.preface}</em></blockquote>`;
    }
    ...
    for (const postId of chapter.postIds) {
      ...
      if (post.publishedAt) {
        chapterContent += `<p><small>${new Date(post.publishedAt).toLocaleDateString('ko-KR')}</small></p>`;
      }
  ...
  const options = {
    ...
    css: `
      body { font-family: 'Malgun Gothic', serif; line-height: 1.8; }

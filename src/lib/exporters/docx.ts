...
export async function generateDocx(
  ...
  sections.push(
    new Paragraph({
      children: [new TextRun({ text: bookInfo.title, size: 56, bold: true })],
      alignment: AlignmentType.CENTER,
      spacing: { before: 2000, after: 400 },
    }),
    new Paragraph({
      children: [new TextRun({ text: bookInfo.author, size: 32 })],
      alignment: AlignmentType.CENTER,
      spacing: { after: 200 },
    })
  );
  ...
  if (bookInfo.description) {
    sections.push(
      new Paragraph({
        children: [new TextRun({ text: bookInfo.description, size: 24, italics: true })],
        alignment: AlignmentType.CENTER,
        spacing: { after: 400 },
      })
    );
  ...
  chapters.forEach((chapter, idx) => {
    sections.push(
      new Paragraph({
        children: [new TextRun({ text: `${idx + 1}. ${chapter.title}`, size: 24 })],
        spacing: { after: 200 },
      })
    );
  });
  ...
  for (const chapter of chapters) {
    ...
    if (chapter.preface) {
      sections.push(
        new Paragraph({
          children: [new TextRun({ text: chapter.preface, size: 24, italics: true })],
          spacing: { after: 400 },
        })
      );
    }
    ...
    for (const postId of chapter.postIds) {
      ...
      if (post.publishedAt) {
        sections.push(
          new Paragraph({
            children: [
              new TextRun({
                text: new Date(post.publishedAt).toLocaleDateString('ko-KR'),
                size: 20,
                color: '888888',
              }),
            ],
      ...
      const contentParagraphs = htmlToDocxParagraphs(post.content);

...
export function ExportModal({
  ...
  async function handleExport() {
    ...
    try {
      const res = await fetch('/api/export', {
        ...
        body: JSON.stringify({
          ...
          bookInfo: {
            ...
            description: bookPreface || project.description,
          },
          bookPreface,
          bookEpilogue,
        }),
      ...
      a.href = url;
      const ext = format === 'docx' ? 'docx' : format === 'pdf' ? 'pdf' : 'epub';
      a.download = `${project.coverTitle || project.title}.${ext}`;
  ...
  const formatInfo = {
    pdf: {
      label: 'PDF',
      description: '인쇄에 최적화된 형식. 레이아웃이 고정됩니다.',
      icon: '📄',
    },
    docx: {
      label: 'Word (DOCX)',
      description: 'Microsoft Word에서 편집 가능한 형식',
      icon: '📝',
    },
    epub: {
      label: 'EPUB',
      description: '전자책 리더기 지원 형식',
      icon: '📚',
    },
  };
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="책 내보내기" size="md">
      <div className="space-y-6">
        ...
        <div className="flex justify-end gap-2">
          ...
          >
            {loading ? '생성 중...' : `${formatInfo[format].label}로 내보내기`}
          </Button>
        </div>
      </div>
    </Modal>

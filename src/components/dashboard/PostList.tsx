...
export function PostList({ projectId, onSelectPost }: PostListProps) {
  ...
  const filtered = posts
    .filter((p) => {
      ...
      const matchFilter =
        filterSelected === 'all' ||
        (filterSelected === 'selected' && p.selected) ||
        (filterSelected === 'unselected' && !p.selected);
      ...
      <div className="p-4 border-b space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-900">
            수집된 글 <span className="text-gray-500 font-normal">({posts.length})</span>
          </h2>
        ...
        <div className="flex gap-2">
          ...
          <select
            ...
            onChange={(e) => setFilterSelected(e.target.value as typeof filterSelected)}
            className="flex-1 px-2 py-1.5 border border-gray-300 rounded text-xs focus:outline-none"
          >
        ...
        {selectedCount > 0 && (
          <p className="text-xs text-blue-600 font-medium">{selectedCount}개 선택됨</p>
        )}
      </div>
              ...
              >
                <div className="flex items-start gap-3">
                  <input
                    ...
                    onChange={(e) => toggleSelect(projectId, post.id, e.target.checked)}

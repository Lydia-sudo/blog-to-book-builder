...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!url || typeof url !== 'string') return NextResponse.json({ error: 'URL이 필요합니다' }, { status: 400 });
    ...
    return NextResponse.json({ error: err instanceof Error ? err.message : '스크래핑 중 오류가 발생했습니다' }, { status: 500 });
  }
}
